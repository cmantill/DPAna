...
int main(int argc,char** argv)
{
    ...
    do {
		...
		double t1; t1 = sqrt(x1*x1 + y1*y1 + z1*z1 + event->postrack->phep[4]*event->postrack->phep[4]);
                GenParticle* ppostrack = new GenParticle( FourVector(x1, y1, z1, t1), event->postrack->idhep, event->postrack->isthep);
                // create negtrack particle
                double z2; z2 = vx[2];
                double t2; t2 = sqrt(x2*x2 + y2*y2 + z2*z2 + event->negtrack->phep[4]*event->negtrack->phep[4]);
                GenParticle* pnegtrack = new GenParticle( FourVector(x2, y2, z2, t2), event->negtrack->idhep, event->negtrack->isthep);
