...
def makePlots(c,events,goodCut,badCut,filename,name,var,nbins,xmin,xmax,cut,forward):
    print goodCut
    print badCut
    print cut
    c.Clear()
    c.Divide(2,2)
    c.cd(1)
    events.Draw("{}>>good({},{},{})".format(var,nbins,xmin,xmax),goodCut+" && "+cut,"")
    gDirectory.Get("good").SetTitle(var+", good events")
    c.cd(2)
    events.Draw("{}>>bad({},{},{})".format(var,nbins,xmin,xmax),badCut+" && "+cut,"")
