...
for sign in [1,2]:
    for events in [nim2events,nim3events]:
        ...
        fiducialcut = "abs({0})>7 && abs({1})>7".format(y_st1, y_st2)
        ...
        c.cd(2)
        events.Draw("{0}:{1}>>h2(100,-150,150,100,-50,50)".format(y_st1,x_st1)," && ".join([trackqualitycut,acceptancecut,quadrantcut,fiducialcut]),"colz")
        c.cd(3)
        events.Draw("{0}:{1}>>h3(100,-150,150,100,-150,150)".format(y_st2,x_st2)," && ".join([trackqualitycut,acceptancecut,quadrantcut,fiducialcut]),"colz")
        c.cd(4)
